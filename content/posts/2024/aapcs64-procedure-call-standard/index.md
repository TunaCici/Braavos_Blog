...
Before a thread executes the `BL` branch with link instruction (basically calling a function) the link register is set to the current `PC` program counter so that when the function returns with `RET` this register would be pointing to the caller function.

The exact value hold in it is the address of the NEXT instruction right after `BL`.
